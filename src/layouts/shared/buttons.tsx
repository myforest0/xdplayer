import {
    ...
    Tooltip,
    useMediaState,
    type TooltipPlacement, ToggleButton,
} from '@vidstack/react';
import {
    ChatCollapseIcon,
    ChatIcon,
...
export function Chat({ tooltipPlacement }: MediaButtonProps) {
    // const isActive = useMediaState('pictureInPicture');
        ...
        <Tooltip.Root>
            <Tooltip.Trigger asChild>
                <ToggleButton className="vds-button">
                    <ChatIcon className="pressed-icon vds-icon" />
                    {/*<ChatCollapseIcon className="not-pressed-icon vds-icon" />*/}
                </ToggleButton>
            </Tooltip.Trigger>
            <Tooltip.Content className="vds-tooltip-content" placement={tooltipPlacement}>
                {/*{isActive ? 'Exit PIP' : 'Enter PIP'}*/}
